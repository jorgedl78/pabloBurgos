   ...
   INCLUDE('ABREPORT.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('COMIS029.INC'),ONCE        !Local module procedure declarations
                     END


ImpresionGuia1 PROCEDURE                                   ! Generated from procedure template - Report
...
encabezado             DETAIL,AT(,,,3135),USE(?encabezado)
                         ...
                         STRING(@s1),AT(3552,135),USE(GUIA:Letra),TRN,CENTER,FONT(,18,,FONT:bold)
                         STRING('ACUSE DE RECIBO N<186>'),AT(4583,115,1333,188),USE(?String6),TRN,FONT(,9,,FONT:bold)
                         STRING(@n08),AT(6458,115),USE(GUIA:Numero),TRN,RIGHT(1),FONT('Courier New',10,,FONT:bold)
                         STRING(@n04),AT(5969,115),USE(GUIA:Lugar),TRN,RIGHT(1),FONT('Courier New',10,,FONT:bold)
                         STRING('Comisiones SRL'),AT(1083,323),USE(?String2),TRN,FONT(,12,,FONT:bold)
                         STRING('Fecha:'),AT(5479,396),USE(?String8),TRN
                         STRING(@d6),AT(5927,396),USE(GUIA:Fecha),RIGHT(1),FONT(,,,FONT:bold)
                         ...
                         STRING('pabloandresburgos@yahoo.com.ar'),AT(1521,844),USE(?String38:2),TRN
                         STRING('ORIGINAL'),AT(6417,865,813,177),USE(?Original),TRN,RIGHT(1)
                         STRING('Alias: pabloysantiagoburgos'),AT(4542,844),USE(?String11:5),TRN,FONT(,,,FONT:bold)
                         ...
                         STRING('Remitente:'),AT(208,1188),USE(?String11),TRN,FONT(,,,FONT:bold)
                         STRING(@s40),AT(958,1188,2573,167),USE(GUIA:NombreRemitente),TRN
                         STRING('Destinatario:'),AT(3854,1188),USE(?String11:2),TRN,FONT(,,,FONT:bold)
                         ...
                         STRING('Observaciones:'),AT(208,2031),USE(?String11:3),TRN,FONT(,,,FONT:bold)
                         STRING('Redespacho:'),AT(3854,2031),USE(?String11:4),TRN,FONT(,,,FONT:bold)
                         ...
                         STRING('** FLETE A COBRAR EN DESTINO **'),AT(3854,2719,2094,146),USE(?Flete),TRN,FONT(,,,FONT:bold)
                         BOX,AT(73,2896,7332,208),USE(?Box2),COLOR(COLOR:Black),FILL(0F4F4F4H)
                         STRING('Cantidad'),AT(115,2938),USE(?String26),TRN,FONT(,,,FONT:bold)
                         STRING('Descripción'),AT(688,2938),USE(?String26:2),TRN,FONT(,,,FONT:bold)
                         STRING('Aforo'),AT(3365,2938),USE(?String26:3),TRN,FONT(,,,FONT:bold)
                         STRING('Valor declarado $'),AT(5500,2938),USE(?String26:5),TRN,FONT(,,,FONT:bold)
                         STRING(@n9.2),AT(6500,2938),USE(GUIA:ValorDeclarado),TRN,RIGHT(1),FONT(,,,FONT:bold)
                       END
...
pie                    DETAIL,AT(,,,1490),USE(?pie)
                         LINE,AT(7400,-1125,0,2120),USE(?Line13:3),COLOR(COLOR:Black)
                         ...
                         STRING('TOTAL'),AT(2594,73),USE(?String33),TRN,FONT(,,,FONT:bold)
                         ...
                         STRING('y contenido. En caso de pérdida o avería no estará obligado a indemnizar'),AT(3823,708),USE(?String45:4),TRN,FONT(,7,,)
                         STRING('. . . . . . . . . . . . . . . . . . '),AT(135,729),USE(?String36:2),TRN
                         STRING('. . . . . . . . . . . . . . . . . .'),AT(1385,729),USE(?String36:3),TRN
                         STRING('de valor, si al tiempo de entrega, el remitente no hubiera declarado su valor'),AT(3823,583),USE(?String45:3),TRN,FONT(,7,,)
                         STRING('Apellido y Nombre'),AT(885,260),USE(?String35),TRN
                         STRING('Documento'),AT(1688,833),USE(?String35:3),TRN
                         LINE,AT(73,990,7332,0),USE(?Line1:5),COLOR(COLOR:Black)
                         STRING('más que el valor declarado.'),AT(3823,833),USE(?String45:5),TRN,FONT(,7,,)
                         STRING('Firma'),AT(563,833),USE(?String35:2),TRN
                         STRING('1)  El comisionista NO se hará RESPONSABLE del DINERO EN EFECTIVO'),AT(3823,188),USE(?String45:7),TRN,FONT(,7,,)
                         STRING('proveniente de contrarreembolso o pagos a efectuar.'),AT(3823,302),USE(?String45:8),TRN,FONT(,7,,)
                         STRING('2)  El comisionista NO se hará RESPONSABLE de documentos y efectos'),AT(3823,458),USE(?String45:2),TRN,FONT(,7,,)
                       END
                     END
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('ImpresionGuia1')
...
SkipDetails BYTE
  CODE
  CLI:CodCliente=GUIA:ClienteFacturar
  ...
  LOOP 2 TIMES
    ...
    LOOP UNTIL EOF(ITEMGUIA)
      ...
      IF CLI:PosicionIVA<>1 THEN
         ITGUIA:Aforo=ITGUIA:Aforo * 1.21 !PARA LOS QUE NO SON INSCRIPTOS QUE LE IMPRIMA LA GUIA CON EL IVA INCLUIDO
      END!IF
