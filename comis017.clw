   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('COMIS017.INC'),ONCE        !Local module procedure declarations
...
UpdateGUIAS1 PROCEDURE                                     ! Generated from procedure template - Window
...
QuickWindow          WINDOW('Update the GUIAS File'),AT(,,374,254),FONT('MS Sans Serif',8,,),COLOR(0BFDFDFH),CENTER,IMM,GRAY,DOUBLE,MDI
                       STRING('REMITO-GUIA'),AT(7,0),USE(?String1),TRN,FONT('Arial',10,0804000H,FONT:bold)
                       STRING('Facturar a:'),AT(83,1),USE(?String9),TRN,FONT(,,,FONT:bold)
                       OPTION,AT(127,-1,117,13),USE(GUIA:FacturarA),TRN
                         RADIO('Remitente'),AT(134,1),USE(?GUIA:FacturarA:Radio1),TRN,VALUE('R')
                         RADIO('Destinatario'),AT(189,1),USE(?GUIA:FacturarA:Radio2),TRN,VALUE('D')
                       END
                       STRING('Pablo A. Burgos Comisiones'),AT(260,2,111,10),USE(?String8),TRN,FONT(,,,FONT:bold+FONT:italic)
                       PANEL,AT(-1,11,376,18),USE(?Panel1),FILL(0804000H),BEVEL(-1)
                       ...
                       PROMPT('Remitente:'),AT(12,33),USE(?GUIA:Cliente:Prompt),TRN,FONT(,,,FONT:bold)
                       CHECK('Es Cliente ?'),AT(56,33),USE(loc:RteEsCliente),TRN,LEFT,VALUE('S','N')
                       ENTRY(@n6.b),AT(130,33,35,10),USE(GUIA:Remitente),SKIP,DISABLE,RIGHT(1),COLOR(0F5F5F5H)
                       BUTTON,AT(115,32,12,11),USE(?SelectRemitente),DISABLE,FLAT,TIP('Seleccionar Remitente'),ICON('C:\Comisiones\ComisionesSRL\botones\lookup.ico')
                       BOX,AT(10,45,173,58),USE(?Box1),ROUND,COLOR(COLOR:Black)
                       PROMPT('Nombre:'),AT(15,49),USE(?GUIA:NombreRemitente:Prompt),TRN
                       ENTRY(@s40),AT(52,49,125,10),USE(GUIA:NombreRemitente),COLOR(0F5F5F5H),UPR
                       PROMPT('Dirección:'),AT(15,62),USE(?GUIA:DireccionRemitente:Prompt),TRN
                       ENTRY(@s30),AT(52,62,125,10),USE(GUIA:DireccionRemitente),COLOR(0F5F5F5H),UPR
                       STRING('Localidad:'),AT(15,75),USE(?String3:2),TRN
                       COMBO(@s30),AT(52,75,125,10),USE(LOC:Localidad),IMM,COLOR(0F5F5F5H),UPR,FORMAT('120L(2)@s30@'),DROP(5),FROM(Queue:FileDropCombo)
                       PROMPT('Teléfono:'),AT(15,88),USE(?GUIA:TelefonoRemitente:Prompt),TRN
                       ENTRY(@s35),AT(52,88,125,10),USE(GUIA:TelefonoRemitente),COLOR(0F5F5F5H),UPR
                       STRING('Destinatario:'),AT(194,33),USE(?String4:2),TRN,FONT(,,,FONT:bold)
                       CHECK('Es Cliente ?'),AT(245,33),USE(loc:DesEsCliente),TRN,LEFT,VALUE('S','N')
                       BUTTON,AT(304,32,12,11),USE(?SelectDestinatario),DISABLE,FLAT,TIP('Seleccionar Destinatario'),ICON('C:\Comisiones\ComisionesSRL\botones\lookup.ico')
                       ENTRY(@n6.b),AT(319,33,35,10),USE(GUIA:Destinatario),SKIP,DISABLE,RIGHT(1),COLOR(0F5F5F5H)
                       BOX,AT(193,45,173,58),USE(?Box1:2),ROUND,COLOR(COLOR:Black)
                       PROMPT('Nombre:'),AT(198,49),USE(?GUIA:NombreDestino:Prompt),TRN
                       ENTRY(@s40),AT(236,49,125,10),USE(GUIA:NombreDestino),COLOR(0F5F5F5H),UPR
                       PROMPT('Dirección:'),AT(198,62),USE(?GUIA:DireccionDestino:Prompt),TRN
                       ENTRY(@s30),AT(236,62,125,10),USE(GUIA:DireccionDestino),COLOR(0F5F5F5H),UPR
                       STRING('Localidad:'),AT(198,75),USE(?String3),TRN
                       COMBO(@s30),AT(236,75,125,10),USE(LOC2:Localidad),IMM,COLOR(0F5F5F5H),UPR,FORMAT('120L(2)@s30@'),DROP(5),FROM(Queue:FileDropCombo:1)
                       PROMPT('Teléfono:'),AT(198,88),USE(?GUIA:TelefonoDestino:Prompt),TRN
                       ENTRY(@s35),AT(236,88,125,10),USE(GUIA:TelefonoDestino),COLOR(0F5F5F5H),UPR
                       PROMPT('Distribuidor:'),AT(12,113),USE(?GUIA:Distribuidor:Prompt),TRN
                       ENTRY(@n3b),AT(58,113,19,10),USE(GUIA:Distribuidor),RIGHT(1),COLOR(0F5F5F5H)
                       BUTTON,AT(80,113,12,11),USE(?SelecDistribuidor),SKIP,FLAT,TIP('Seleccionar Distribuidor'),ICON('C:\Comisiones\ComisionesSRL\botones\lookup.ico')
                       STRING(@s35),AT(96,113,135,10),USE(DIS:Nombre),TRN,FONT(,,0A00000H,)
                       PROMPT('Redespacho:'),AT(12,129),USE(?GUIA:Redespacho:Prompt),TRN
                       ENTRY(@n3b),AT(58,129,19,10),USE(GUIA:Redespacho),RIGHT(1),COLOR(0F5F5F5H)
                       BUTTON,AT(80,129,12,11),USE(?SelecTransporte),SKIP,FLAT,TIP('Seleccionar Transporte'),KEY(F3Key),ICON('C:\Comisiones\ComisionesSRL\botones\lookup.ico')
                       ...
                       ENTRY(@n15.2),AT(237,115,60,10),USE(GUIA:ContraReembolso),RIGHT(1),COLOR(0F5F5F5H)
                       ...
                       PROMPT('Observación:'),AT(12,145),USE(?GUIA:Observacion:Prompt),TRN,FONT(,,,FONT:bold)
                       OPTION('Flete'),AT(305,105,58,44),USE(GUIA:Flete),BOXED
                         RADIO('Origen'),AT(313,115),USE(?GUIA:Flete:Radio1),TRN,VALUE('R')
                       ...
                       BUTTON('Agregar'),AT(20,188,13,12),USE(?Insert),FLAT,LEFT,TIP('Agregar Item'),ICON('C:\Comisiones\ComisionesSRL\botones\Insert.ico')
                       BUTTON('Modificar'),AT(20,203,13,12),USE(?Change),SKIP,FLAT,LEFT,TIP('Modificar Item'),ICON('C:\Comisiones\ComisionesSRL\botones\Edit.ico')
                       BUTTON('Borrar'),AT(20,218,13,12),USE(?Delete),SKIP,FLAT,LEFT,TIP('Borrar Item'),ICON('C:\Comisiones\ComisionesSRL\botones\Delete.ico')
                       LIST,AT(37,187,239,43),USE(?List),IMM,NOBAR,COLOR(0F1F1F1H),FORMAT('34R(4)|F~Cantidad~C(0)@n7.@145L(2)|FM~Descripción~@s40@40R(3)|FM~Aforo~L(2)@n15.' &|
   '2@'),FROM(Queue:Browse)
                       PROMPT('Valor Declarado:'),AT(295,188),USE(?GUIA:ValorDeclarado:Prompt),TRN
                       ENTRY(@n15.2),AT(295,197,53,10),USE(GUIA:ValorDeclarado),RIGHT(2),COLOR(0F5F5F5H)
                       PROMPT('Importe:'),AT(295,211),USE(?GUIA:Importe:Prompt),TRN
                       ENTRY(@n15.2),AT(295,220,53,10),USE(GUIA:Importe),RIGHT(2),COLOR(0F5F5F5H)
                       BUTTON(' OK'),AT(127,238,55,15),USE(?OK),LEFT,FONT(,,,FONT:bold),ICON('C:\Comisiones\ComisionesSRL\botones\Ok1.ico')
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('UpdateGUIAS1')
