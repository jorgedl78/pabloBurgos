   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('COMIS085.INC'),ONCE        !Local module procedure declarations
...
UpdateFACTURAS3_fe PROCEDURE                               ! Generated from procedure template - Window
...
QuickWindow          WINDOW('Update the FACTURAS File'),AT(,,377,274),FONT('MS Sans Serif',8,COLOR:Black,),COLOR(0BFDFDFH),CENTER,IMM,GRAY,DOUBLE,MDI
                       STRING('FACTURA'),AT(168,15),USE(?String12),TRN,FONT('Arial',10,COLOR:White,FONT:bold)
                       STRING('Cobrar a:'),AT(17,1),USE(?String8),TRN,FONT(,,COLOR:Black,FONT:bold)
                       ...
                       END
                       STRING('Forma Pago:'),AT(203,1),USE(?String8:2),TRN,FONT(,,COLOR:Black,FONT:bold)
                       ...
                       CHECK('Factura electrónica'),AT(12,28),USE(loc:FacturaElectronica),TRN,FONT(,,COLOR:White,FONT:bold,CHARSET:ANSI)
                       PROMPT('Remitente:'),AT(11,46),USE(?FAC:Remitente:Prompt),TRN,FONT(,,COLOR:Black,FONT:bold)
                       CHECK('Es Cliente?'),AT(57,46),USE(loc:RteEsCliente),TRN,LEFT,VALUE('S','N')
                       ...
                       ENTRY(@n6.b),AT(131,46,33,10),USE(FAC:Remitente),SKIP,DISABLE,RIGHT(1),COLOR(0F5F5F5H)
                       BOX,AT(11,57,167,64),USE(?BoxRemitente),ROUND,COLOR(COLOR:Black)
                       PROMPT('Nombre:'),AT(17,60),USE(?FAC:NombreRemitente:Prompt),TRN
                       ENTRY(@s40),AT(54,60,120,10),USE(FAC:NombreRemitente),COLOR(0F5F5F5H),UPR
                       PROMPT('CUIT:'),AT(17,72),USE(?FAC:CUITRemitente:Prompt),TRN
                       ENTRY(@P##-########-#Pb),AT(54,72,60,10),USE(FAC:CUITRemitente),COLOR(0F5F5F5H),UPR
                       PROMPT('Dirección:'),AT(17,84),USE(?FAC:DireccionRemitente:Prompt),TRN
                       ENTRY(@s30),AT(54,84,120,10),USE(FAC:DireccionRemitente),COLOR(0F5F5F5H),UPR
                       STRING('Localidad:'),AT(17,96),USE(?Localidad),TRN
                       COMBO(@s30),AT(54,96,120,10),USE(LOC:Localidad),IMM,COLOR(0F5F5F5H),UPR,FORMAT('120L(2)@s30@'),DROP(5),FROM(Queue:FileDropCombo)
                       PROMPT('Teléfono:'),AT(17,108),USE(?FAC:TelefonoRemitente:Prompt),TRN
                       ENTRY(@s35),AT(54,108,120,10),USE(FAC:TelefonoRemitente),COLOR(0F5F5F5H),UPR
                       BOX,AT(201,57,167,64),USE(?BoxDestino),ROUND,COLOR(COLOR:Black)
                       PROMPT('Destinatario:'),AT(202,46),USE(?FAC:Destinatario:Prompt),TRN,FONT(,,COLOR:Black,FONT:bold)
                       CHECK('Es Cliente?'),AT(252,46),USE(loc:DesEsCliente),TRN,LEFT,VALUE('S','N')
                       ...
                       ENTRY(@n6.b),AT(322,46,33,10),USE(FAC:Destinatario),SKIP,DISABLE,RIGHT(1),COLOR(0F5F5F5H)
                       PROMPT('Nombre:'),AT(207,60),USE(?FAC:NombreDestino:Prompt),TRN
                       ENTRY(@s40),AT(244,60,120,10),USE(FAC:NombreDestino),COLOR(0F5F5F5H),UPR
                       PROMPT('CUIT:'),AT(207,72),USE(?FAC:CUITDestino:Prompt),TRN
                       ENTRY(@P##-########-#Pb),AT(244,72,60,10),USE(FAC:CUITDestino),COLOR(0F5F5F5H)
                       PROMPT('Dirección:'),AT(207,84),USE(?FAC:DireccionDestino:Prompt),TRN
                       ENTRY(@s30),AT(244,84,120,10),USE(FAC:DireccionDestino),COLOR(0F5F5F5H),UPR
                       STRING('Localidad:'),AT(207,96),USE(?Localidad:2),TRN
                       COMBO(@s30),AT(244,96,120,10),USE(LOC2:Localidad),IMM,COLOR(0F5F5F5H),UPR,FORMAT('120L(2)@s30@'),DROP(5),FROM(Queue:FileDropCombo:1)
                       PROMPT('Teléfono:'),AT(207,108),USE(?FAC:TelefonoDestino:Prompt),TRN
                       ENTRY(@s35),AT(244,108,120,10),USE(FAC:TelefonoDestino),COLOR(0F5F5F5H),UPR
                       SHEET,AT(9,124,359,72),USE(?Sheet1),ABOVE(68)
                         TAB('Contado'),USE(?Contado)
                           LIST,AT(64,143,250,35),USE(?List:3),IMM,COLOR(0F5F5F5H),FORMAT('40R(3)|F~Cantidad~L(2)@n10.2@160L(2)|F~Descripción~@s40@40R(3)|F~Importe~L(2)@n1' &|
   '0.2@'),FROM(Queue:Browse:2)
                           BUTTON('Agregar'),AT(64,180,42,12),USE(?Insert),SKIP
                           BUTTON('Modificar'),AT(106,180,42,12),USE(?Change),SKIP
                           BUTTON('Borrar'),AT(148,180,42,12),USE(?Delete),SKIP
                           STRING(@n-11.2),AT(262,180),USE(loc:TotalContado),TRN,RIGHT(1),FONT(,,COLOR:Black,FONT:bold)
                         END
                         TAB('Cuenta Corriente'),USE(?CuentaCorriente)
                           STRING('Remitos-Guía en Cta. Cte.'),AT(16,140),USE(?String3),TRN,FONT(,,COLOR:Green,FONT:bold)
                           STRING('Remitos a Facturar'),AT(203,140),USE(?String4),TRN,FONT(,,COLOR:Maroon,FONT:bold)
                           LIST,AT(16,148,158,35),USE(?List),IMM,NOBAR,VSCROLL,COLOR(0F5F5F5H,COLOR:Black,0FFFF80H),FORMAT('47R(3)|F~Fecha~L(2)@d6@[8CF@s1@21R(2)F@n04@33R(2)|F@n08@](65)|F~Comprobante~L(2)' &|
   '48R(3)|F~Importe~C(0)@n12.2@'),FROM(Queue:Browse)
                           LIST,AT(203,148,158,35),USE(?List:2),IMM,NOBAR,VSCROLL,COLOR(0F5F5F5H,COLOR:Black,0FFFF80H),FORMAT('47R(3)|F~Fecha~L(2)@d6@[8CF@s1@21R(2)F@n04@39R(2)|F@n08@](65)|F~Comprobante~L(2)' &|
   '48R(3)|F~Importe~C(0)@n12.2@'),FROM(Queue:Browse:1)
                           BUTTON,AT(180,153,17,14),USE(?Facturar),SKIP,FLAT,TIP('Facturar'),ICON('C:\1L\Comisiones\botones\aplica.ico')
                           BUTTON,AT(180,169,17,14),USE(?Volver),SKIP,FLAT,TIP('Volver a Cta. Cte.'),ICON('C:\1L\Comisiones\botones\desaplica.ico')
                           STRING(@n-11.2),AT(120,185),USE(loc:TotalCtaCte),TRN,RIGHT(1),FONT(,,COLOR:Black,FONT:bold)
                           STRING(@n-11.2),AT(307,185),USE(loc:TotalFacturar),TRN,RIGHT(1),FONT(,,COLOR:Black,FONT:bold)
                         END
                       END
                       OPTION('Tipo de Servicio'),AT(135,198,63,31),USE(FAC:TipoServicio),BOXED
                         RADIO('Transporte'),AT(146,206),USE(?FAC:TipoServicio:Radio1),TRN,VALUE('T')
                         RADIO('Comisión'),AT(146,216),USE(?FAC:TipoServicio:Radio2),TRN,VALUE('C')
                       END
                       PROMPT('Valor Declarado:'),AT(141,230),USE(?FAC:ValorDeclarado:Prompt),TRN
                       ENTRY(@n10.2),AT(141,238,53,10),USE(FAC:ValorDeclarado),RIGHT(1),COLOR(0F5F5F5H)
                       OPTION('Flete'),AT(204,198,53,51),USE(FAC:Flete),BOXED
                         RADIO('Origen'),AT(209,211),USE(?FAC:Flete:Radio1),TRN,VALUE('R')
                         RADIO('Intermedio'),AT(209,222),USE(?FAC:Flete:Radio2),TRN,VALUE('I')
                         RADIO('Destino'),AT(209,235),USE(?FAC:Flete:Radio3),TRN,VALUE('D')
                       END
                       BOX,AT(264,200,104,50),USE(?Box1),ROUND,COLOR(COLOR:Black),FILL(0E2E2E2H)
                       PROMPT('Flete Básico:'),AT(268,206),USE(?FAC:Neto:Prompt),TRN
                       ENTRY(@n-11.2),AT(315,206,48,10),USE(FAC:Neto),SKIP,RIGHT(1),READONLY
                       PROMPT('Seguro:'),AT(284,220),USE(?FAC:Seguro:Prompt),TRN
                       ENTRY(@n10.2),AT(315,220,48,10),USE(FAC:Seguro),RIGHT(1),COLOR(0F5F5F5H)
                       PROMPT('TOTAL:'),AT(279,234),USE(?FAC:Importe:Prompt),TRN,FONT(,,COLOR:Black,FONT:bold)
                       ENTRY(@n-11.2),AT(315,234,48,10),USE(FAC:Importe),SKIP,RIGHT(1),FONT(,,,FONT:bold),COLOR(0C8FFC8H),READONLY
                       STRING('Observación:'),AT(12,198),USE(?String5),TRN
                       TEXT,AT(11,207,117,41),USE(FAC:Observacion),SKIP,BOXED,VSCROLL,FONT(,,COLOR:Red,FONT:bold),COLOR(COLOR:Silver),UPR
                       LINE,AT(9,253,357,0),USE(?Line1),COLOR(COLOR:Gray),LINEWIDTH(2)
                       BUTTON('  OK'),AT(130,256,55,15),USE(?OK),LEFT,FONT(,,COLOR:Black,FONT:bold),ICON('C:\1L\Comisiones\botones\Ok1.ico')
                       BUTTON('Cancelar'),AT(193,256,55,15),USE(?Cancel),LEFT,KEY(EscKey),ICON('C:\1L\Comisiones\botones\Cancel1.ico')
                     END
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('UpdateFACTURAS3_fe')
  ...
  Relate:LOCALIDA.Open                                     ! File FACTURAS used by this procedure, so make sure it's RelationManager is open
  Relate:LOCALIDAD2.Open                                   ! File FACTURAS used by this procedure, so make sure it's RelationManager is open
