   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('COMIS003.INC'),ONCE        !Local module procedure declarations
                       INCLUDE('COMIS016.INC'),ONCE        !Req'd for module callout resolution
                       INCLUDE('COMIS017.INC'),ONCE        !Req'd for module callout resolution
...
GUIAS_EMPRESA1 PROCEDURE                                   ! Generated from procedure template - Window
...
QuickWindow          WINDOW('REMITOS - GUIAS'),AT(,,457,254),FONT('MS Sans Serif',8,,),COLOR(0E4EBEFH),CENTER,IMM,ICON('C:\Comisiones\ComisionesSRL\botones\movi.ico'),SYSTEM,GRAY,DOUBLE,MDI
                       PANEL,AT(-1,5,459,18),USE(?Panel2),FILL(0804000H),BEVEL(-1)
                       STRING('1'),AT(4,6,19,15),USE(?String5),TRN,CENTER,FONT('Arial Black',12,COLOR:White,FONT:bold)
                       STRING('PABLO A. BURGOS  Comisiones'),AT(140,7),USE(?String2),TRN,FONT(,12,COLOR:White,FONT:bold)
                       BUTTON,AT(420,83,24,16),USE(?Redespacho),SKIP,FLAT,TIP('Ver Redespacho'),ICON('C:\Comisiones\ComisionesSRL\botones\camion2.ico')
   ...
   '|FM*~Remitente~@s40@4C|F*~Flete~@s1@'),FROM(Queue:Browse:1)
                       BUTTON('Alta'),AT(297,45,55,33),USE(?Insert:2),SKIP,LEFT,ICON('C:\Comisiones\ComisionesSRL\botones\agregar.ico')
                       BUTTON('Modificar'),AT(357,45,55,15),USE(?Change:2),SKIP,LEFT,ICON('C:\Comisiones\ComisionesSRL\botones\modificar.ico')
                       BUTTON('Borrar'),AT(357,63,55,15),USE(?Delete:2),SKIP,LEFT,ICON('C:\Comisiones\ComisionesSRL\botones\borrar.ico')
                       STRING('DESTINATARIO:'),AT(294,90),USE(?String1),TRN,FONT(,,,FONT:bold)
                       ...
                       PROMPT('Remito:'),AT(372,164),USE(?GUIA:RemitoCliente:Prompt)
                       ENTRY(@s10),AT(399,163,47,10),USE(GUIA:RemitoCliente),SKIP,RIGHT(1),TIP('Remito del Cliente'),READONLY
                       PROMPT('OBSERVACION:'),AT(294,164),USE(?GUIA:Observacion:Prompt),TRN,FONT(,,,FONT:bold)
                       TEXT,AT(294,175,151,51),USE(GUIA:Observacion),SKIP,BOXED,VSCROLL,FONT(,,0400080H,),COLOR(0E1E1E1H),READONLY
                       BUTTON('  Cumplir'),AT(70,155,54,14),USE(?Cumplir),SKIP,LEFT,ICON('C:\Comisiones\ComisionesSRL\botones\marcar.ico')
                       BUTTON(' Imprimir'),AT(12,155,54,14),USE(?Imprimir),SKIP,LEFT,KEY(F2Key),ICON('C:\Comisiones\ComisionesSRL\botones\imprimir.ICO')
                       LIST,AT(12,175,276,51),USE(?List),IMM,SKIP,NOBAR,FORMAT('40R(5)|F~Cantidad~L(2)@n7.@179L(2)|F~Descripción~@s35@40R(3)|F~Aforo~L(2)@n10.2@'),FROM(Queue:Browse)
                       SHEET,AT(7,27,443,205),USE(?CurrentTab),ABOVE(56)
                         TAB('Por &Fecha'),USE(?Tab:1)
                           PROMPT('Fecha:'),AT(135,158),USE(?GUIA:Fecha:Prompt),TRN
                           ENTRY(@d6b),AT(166,158,49,10),USE(GUIA:Fecha),RIGHT(1),FONT(,,0A00000H,,CHARSET:ANSI)
                         END
                         TAB('Por &Número'),USE(?Tab:2)
                           PROMPT('Número:'),AT(135,158),USE(?GUIA:Numero:Prompt),TRN
                           ENTRY(@n_8b),AT(166,158,49,10),USE(GUIA:Numero),RIGHT(1)
                         END
                         TAB('Por &Cliente'),USE(?Tab:3)
                           BUTTON(' Seleccionar Cliente...'),AT(129,155,94,14),USE(?SelectCLIENTE),SKIP,LEFT,ICON('C:\Comisiones\ComisionesSRL\botones\buscar.ico')
                         END
                       END
                       BUTTON('    Cerrar'),AT(201,236,55,15),USE(?Close),SKIP,RIGHT,ICON('C:\Comisiones\ComisionesSRL\botones\sale.ico')
                       BUTTON,AT(394,83,24,16),USE(?ContraReembolso),SKIP,FLAT,TIP('Ver Contra Reembolso'),ICON('C:\Comisiones\ComisionesSRL\botones\cheque.ico')
                       STRING('F2 - Imprime Guía'),AT(10,234),USE(?String3)
                     END
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('GUIAS_EMPRESA1')
  ...
  BRW1.SetFilter('(GUIA:Empresa = 1)')                     ! Apply filter expression to browse
