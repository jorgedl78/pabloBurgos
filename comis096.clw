   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('COMIS096.INC'),ONCE        !Local module procedure declarations
                       ...
                       INCLUDE('COMIS097.INC'),ONCE        !Req'd for module callout resolution
                       INCLUDE('COMIS098.INC'),ONCE        !Req'd for module callout resolution
                     END


RECIBOS_EMPRESA3 PROCEDURE                                 ! Generated from procedure template - Window
...
QuickWindow          WINDOW('RECIBOS'),AT(,,424,237),FONT('MS Sans Serif',8,,),COLOR(0E4EBEFH),CENTER,IMM,ICON('C:\Comisiones\ComisionesSRL\botones\cheque.ico'),SYSTEM,GRAY,DOUBLE,MDI
                       ...
                       STRING('PABLO'),AT(203,6,57,20),USE(?String5),TRN,FONT('Arial Black',16,COLOR:White,FONT:italic,CHARSET:ANSI)
                       STRING('Trans'),AT(164,5),USE(?String1),TRN,FONT('Brush Script MT',20,COLOR:White,)
   ...
   'R(3)|F*~Saldo~C(0)@n-13.2@'),FROM(Queue:Browse:1)
                       ...
                       SHEET,AT(7,30,409,182),USE(?CurrentTab),ABOVE(54)
                         TAB('Por &Fecha'),USE(?Tab:1)
                           ...
                           ENTRY(@d6b),AT(187,177,49,10),USE(FAC:Fecha),RIGHT(1),FONT(,,COLOR:Green,,CHARSET:ANSI)
                         END
                         TAB('Por &Número'),USE(?Tab:2)
                           PROMPT('Número:'),AT(155,177),USE(?FAC:Numero:Prompt),TRN
                           ENTRY(@n_8b),AT(187,177,49,10),USE(FAC:Numero),RIGHT(1),FONT(,,COLOR:Green,,CHARSET:ANSI)
                         END
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('RECIBOS_EMPRESA3')
  ...
  BRW1.SetFilter('(FAC:Empresa = 3 AND FAC:Comprobante = ''REC'')') ! Apply filter expression to browse
