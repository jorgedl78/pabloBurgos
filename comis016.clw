   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('COMIS016.INC'),ONCE        !Local module procedure declarations
                       INCLUDE('COMIS013.INC'),ONCE        !Req'd for module callout resolution
                     END


SelectCLIENTES PROCEDURE                                   ! Generated from procedure template - Window
...
QuickWindow          WINDOW('Selección de Cliente'),AT(,,220,193),FONT('MS Sans Serif',8,,),COLOR(0E2F1F1H),CENTER,IMM,SYSTEM,GRAY,DOUBLE,MDI
                       LIST,AT(12,26,194,139),USE(?Browse:1),IMM,VSCROLL,COLOR(0FFFF80H,COLOR:Black,080FF80H),FORMAT('34R(3)|~Código~L(2)@n6.@165L(2)|~Nombre~@s40@'),FROM(Queue:Browse:1)
                       BUTTON('Alta'),AT(8,174,42,15),USE(?Insert),SKIP,LEFT,ICON('C:\Comisiones\ComisionesSRL\botones\agregar.ico')
                       BUTTON('Seleccionar'),AT(75,174,113,15),USE(?Select:2),FONT(,,,FONT:bold)
                       SHEET,AT(6,5,208,165),USE(?CurrentTab),WIZARD
                         TAB('Por Nombre'),USE(?Tab:1)
                           STRING(@s40),AT(50,12),USE(CLI:Nombre),TRN,FONT(,,0A00000H,)
                         END
                       END
                     END
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('SelectCLIENTES')
  ...
  BRW1::Sort0:StepClass.Init(+ScrollSort:AllowAlpha,ScrollBy:Runtime) ! Moveable thumb based upon CLI:Nombre for sort order 1
  BRW1.AddSortOrder(BRW1::Sort0:StepClass,CLI:Por_Nombre)  ! Add the sort order for CLI:Por_Nombre for sort order 1
