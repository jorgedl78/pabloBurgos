   ...
   INCLUDE('ABREPORT.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('COMIS023.INC'),ONCE        !Local module procedure declarations
                     END


HojaRuta PROCEDURE                                         ! Generated from procedure template - Report
...
report               REPORT,AT(656,948,7135,10146),PAPER(PAPER:A4),PRE(RPT),FONT('Arial',8,,FONT:regular),THOUS
                       HEADER,AT(656,323,7146,594),FONT('Arial',9,,FONT:regular)
                         STRING(@d6),AT(3927,42),USE(GUIA:Fecha),TRN,FONT(,,,FONT:bold)
                         STRING(@N3),AT(6677,42),USE(ReportPageNumber),TRN
                         BOX,AT(73,344,7000,208),USE(?Box1),ROUND,COLOR(COLOR:Black)
                         STRING('Redespacho'),AT(5083,365),USE(?String5:3),TRN,FONT(,,,FONT:bold)
                         STRING('Remitente'),AT(260,365),USE(?String5),TRN,FONT(,,,FONT:bold)
                         STRING('Destinatario'),AT(2750,365),USE(?String5:2),TRN,FONT(,,,FONT:bold)
                         STRING('PAGINA:'),AT(6125,42),USE(?String4),TRN
                         STRING('HOJA DE RUTA DEL DIA'),AT(2510,42),USE(?String1),TRN,FONT(,,,FONT:bold)
                       END
break1                 BREAK(GUIA:Numero)
                         HEADER,AT(,,,167)
                           STRING('Remito Cliente:'),AT(2792,21,792,177),USE(?RemitoCliente),TRN,FONT(,,,FONT:italic)
                           STRING(@s1),AT(1302,21),USE(GUIA:Letra),TRN,FONT(,,,FONT:bold)
                           STRING('Contra Reembolso:'),AT(5406,21),USE(?Reembolso),TRN,FONT(,,,FONT:italic)
                           STRING(@n10.2b),AT(6438,21),USE(GUIA:ContraReembolso),TRN
                           STRING(@n04),AT(1396,21),USE(GUIA:Lugar),TRN,RIGHT(1),FONT('Courier New',8,,FONT:bold)
                           STRING(@n08),AT(1781,21),USE(GUIA:Numero),TRN,RIGHT(1),FONT('Courier New',8,,FONT:bold)
                           STRING(@s10b),AT(3635,21,677,177),USE(GUIA:RemitoCliente),TRN
                           STRING('REMITO-GUIA NRO.:'),AT(94,21),USE(?String8),TRN,FONT(,,,FONT:bold)
                         END
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('HojaRuta')
  ...
  ThisReport.SetFilter('GUIA:Impresa <<> ''A''')
