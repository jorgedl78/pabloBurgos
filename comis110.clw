   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('COMIS110.INC'),ONCE        !Local module procedure declarations
                       INCLUDE('COMIS014.INC'),ONCE        !Req'd for module callout resolution
                     END


UpdateTRANSLOC PROCEDURE                                   ! Generated from procedure template - Window
...
QuickWindow          WINDOW('Form TRANSLOC'),AT(,,241,142),FONT('MS Sans Serif',8,,FONT:regular),CENTER,IMM,SYSTEM,GRAY,DOUBLE,MDI
                       SHEET,AT(7,7,227,114),USE(?CurrentTab),WIZARD
                         TAB('&General'),USE(?Tab:1)
                           PROMPT('Transporte:'),AT(22,19),USE(?TRL:Transporte:Prompt),TRN
                           ENTRY(@n3b),AT(64,19,24,10),USE(TRL:Transporte),RIGHT(1),REQ
                           BUTTON('...'),AT(92,18,12,11),USE(?CallLookup),SKIP,TIP('Seleccionar Transporte')
                           STRING(@s30),AT(108,19),USE(TRA:Denominacion),TRN,FONT(,,0A00000H,,CHARSET:ANSI)
                           STRING('Localidad:'),AT(25,34),USE(?String2),TRN
                           COMBO(@s20),AT(64,34,132,10),USE(LOC:Localidad),IMM,REQ,UPR,FORMAT('120L(2)|@s30@'),DROP(5),FROM(Queue:FileDropCombo:1)
                           STRING('Días:'),AT(41,58),USE(?String3)
                           CHECK('L'),AT(64,58),USE(TRL:Lunes),VALUE('X','')
                           CHECK('M'),AT(87,58),USE(TRL:Martes),VALUE('X','')
                           CHECK('M'),AT(112,58),USE(TRL:Miercoles),VALUE('X','')
                           CHECK('J'),AT(137,58),USE(TRL:Jueves),VALUE('X','')
                           CHECK('V'),AT(159,58),USE(TRL:Viernes),VALUE('X','')
                           CHECK('S'),AT(183,58),USE(TRL:Sabado),VALUE('X','')
                           CHECK('D'),AT(207,58),USE(TRL:Domingo),VALUE('X','')
                           PROMPT('Observación:'),AT(16,77),USE(?TRL:Observacion:Prompt),TRN
                           TEXT,AT(15,86,211,28),USE(TRL:Observacion),SKIP,FONT(,,0A00000H,,CHARSET:ANSI),COLOR(0E0EFEFH)
                         END
                       END
                       BUTTON(' OK'),AT(66,125,52,15),USE(?OK),LEFT,FONT(,,,FONT:bold),ICON('C:\Comisiones\ComisionesSRL\botones\Ok1.ico')
                       BUTTON('Cancelar'),AT(122,125,52,15),USE(?Cancel),LEFT,ICON('C:\Comisiones\ComisionesSRL\botones\Cancel1.ico')
                     END
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('UpdateTRANSLOC')
  SELF.Request = GlobalRequest                             ! Store the incoming request
  ReturnValue = PARENT.Init()
  IF ReturnValue THEN RETURN ReturnValue.
  SELF.FirstField = ?TRL:Transporte:Prompt
  ...
  SELF.AddHistoryField(?TRL:Transporte,1)
  ...
  IF SELF.Request = ViewRecord                             ! Configure controls for View Only mode
    ?TRL:Transporte{PROP:ReadOnly} = True
    DISABLE(?CallLookup)
...
Looped BYTE
  ...
  ReturnValue = PARENT.TakeAccepted()
    CASE ACCEPTED()
    OF ?TRL:Transporte
      IF TRL:Transporte OR ?TRL:Transporte{Prop:Req}
        TRA:CodTransporte = TRL:Transporte
        IF Access:TRANSPOR.TryFetch(TRA:Por_Codigo)
          IF SELF.Run(1,SelectRecord) = RequestCompleted
            TRL:Transporte = TRA:CodTransporte
          ELSE
            SELECT(?TRL:Transporte)
            CYCLE
          END
        END
      END
