   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('COMIS087.INC'),ONCE        !Local module procedure declarations
...
UpdateRECIBOS1 PROCEDURE                                   ! Generated from procedure template - Window
...
QuickWindow          WINDOW('Update the FACTURAS File'),AT(,,377,237),FONT('MS Sans Serif',8,,),COLOR(0BFDFDFH),CENTER,IMM,GRAY,DOUBLE,MDI
                       STRING('RECIBO'),AT(172,1),USE(?String12),TRN,FONT('Arial',10,0804000H,FONT:bold)
                       STRING('Pablo A. Burgos Comisiones'),AT(261,2,113,10),USE(?String9),TRN,FONT(,,,FONT:bold+FONT:italic)
                       PANEL,AT(-25,13,403,23),USE(?Panel1),FILL(0804000H),BEVEL(-1)
                       ...
                       BUTTON,AT(82,45,12,11),USE(?SelectCliente),FLAT,TIP('Seleccionar Cliente'),ICON('C:\Comisiones\ComisionesSRL\botones\lookup.ico')
                       ...
                       BUTTON,AT(82,62,12,11),USE(?CallLookup),SKIP,FLAT,TIP('Seleccionar Distribuidor'),ICON('C:\Comisiones\ComisionesSRL\botones\lookup.ico')
   ...
   'echa~L(2)@d6b@52R(3)|FM~Importe~L(2)@n-13.2@'),FROM(Queue:Browse:2)
                           BUTTON,AT(70,148,14,12),USE(?Insert),SKIP,FLAT,TIP('Agregar'),ICON('C:\Comisiones\ComisionesSRL\botones\Insert.ico')
                           BUTTON,AT(50,148,14,12),USE(?Change),SKIP,FLAT,TIP('Modificar'),ICON('C:\Comisiones\ComisionesSRL\botones\Edit.ico')
                           BUTTON,AT(30,148,14,12),USE(?Delete),SKIP,FLAT,TIP('Borrar'),ICON('C:\Comisiones\ComisionesSRL\botones\Delete.ico')
                         END
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('UpdateRECIBOS1')
  ...
  BRW2.SetFilter('(REC:Empresa = 1 AND (REC:Importe + REC:Aplicado) <<> 0 AND REC:Comprobante = ''FAC'')') ! Apply filter expression to browse
