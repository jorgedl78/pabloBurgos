   ...
   INCLUDE('ABREPORT.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('COMIS116.INC'),ONCE        !Local module procedure declarations
                     END


ContraReembolsosCobrados PROCEDURE                         ! Generated from procedure template - Report
...
report               REPORT,AT(656,948,7135,10146),PAPER(PAPER:A4),PRE(RPT),FONT('Arial',8,COLOR:Black,FONT:regular),THOUS
                       HEADER,AT(656,323,7146,594),FONT('Arial',9,,FONT:regular)
                         STRING(@d6b),AT(3208,156),USE(glo:FechaHasta),TRN,RIGHT(1),FONT(,,,FONT:bold)
                         STRING(@N3),AT(6677,156),USE(ReportPageNumber),TRN
                         BOX,AT(31,344,7083,208),USE(?Box1),ROUND,COLOR(COLOR:Black)
                         STRING('Remitente'),AT(1938,365),USE(?String5),TRN,FONT(,,,FONT:bold)
                         STRING('Destinatario'),AT(3875,365),USE(?String5:2),TRN,FONT(,,,FONT:bold)
                         STRING('Importe'),AT(6510,365),USE(?String5:5),TRN,FONT(,,,FONT:bold)
                         STRING('Flete'),AT(6000,365),USE(?String5:6),TRN,FONT(,,,FONT:bold)
                         STRING('Fecha Guía'),AT(73,365),USE(?String5:3),TRN,FONT(,,,FONT:bold)
                         STRING('Número Guía'),AT(813,365),USE(?String5:4),TRN,FONT(,,,FONT:bold)
                         STRING('PAGINA:'),AT(6125,156),USE(?String4),TRN
                         STRING('LISTADO DE CONTRA REEMBOLSOS COBRADOS EL'),AT(52,156),USE(?String1),TRN
                       END
detalle                DETAIL,AT(10,,7135,385),USE(?detalle)
                         ...
                         STRING(@n04),AT(844,21),USE(GUIA:Lugar),TRN,RIGHT(1),FONT('Courier New',8,,)
                         STRING(@n08),AT(1188,21),USE(GUIA:Numero),TRN,RIGHT(1),FONT('Courier New',8,,)
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('ContraReembolsosCobrados')
  ...
  ThisReport.SetFilter('GUIA:Cumplida = glo:FechaHasta AND GUIA:ContraReembolso')
