   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('COMIS027.INC'),ONCE        !Local module procedure declarations
                       INCLUDE('COMIS016.INC'),ONCE        !Req'd for module callout resolution
                       INCLUDE('COMIS028.INC'),ONCE        !Req'd for module callout resolution
                       INCLUDE('COMIS031.INC'),ONCE        !Req'd for module callout resolution
                       INCLUDE('COMIS077.INC'),ONCE        !Req'd for module callout resolution
                       INCLUDE('COMIS120.INC'),ONCE        !Req'd for module callout resolution
                       INCLUDE('COMIS133.INC'),ONCE        !Req'd for module callout resolution
                       INCLUDE('COMIS135.INC'),ONCE        !Req'd for module callout resolution
                     END


FACTURAS_EMPRESA1 PROCEDURE                                ! Generated from procedure template - Window
...
FAC:Comprobante_NormalFG LONG                         !Normal forground color
FAC:Comprobante_NormalBG LONG                         !Normal background color
FAC:Comprobante_SelectedFG LONG                       !Selected forground color
FAC:Comprobante_SelectedBG LONG                       !Selected background color
...
QuickWindow          WINDOW('FACTURAS'),AT(,,424,237),FONT('MS Sans Serif',8,,),COLOR(0E4EBEFH),CENTER,IMM,ICON('C:\Comisiones\ComisionesSRL\botones\facturacion.ico'),SYSTEM,GRAY,DOUBLE,MDI
                       ...
                       BUTTON('Electrónica'),AT(158,192,61,15),USE(?Imprimir:2),SKIP,LEFT,KEY(F2Key),ICON('C:\Comisiones\ComisionesSRL\botones\imprimir.ICO')
                       LIST,AT(13,48,398,123),USE(?Browse:1),IMM,VSCROLL,COLOR(0F8F8F8H,COLOR:White,COLOR:Green),FORMAT('53R(3)|F*J~Fecha~C(0)@d6@[21CF*~Tipo~@s3@21CF*~Letra~@s1@23R(4)F*~Lugar~C(0)@n04' &|
   '@28R(4)|F*~Número~C(0)@n08@](106)|F~Comprobante~[41R(3)|F*~Código~C(0)@n6.b@?158' &|
   'L(2)|F*~Nombre~@s40@7C|F*@s1@](203)|F~Cliente~46R(4)|*~Importe~C(0)@n-11.2@40R(3' &|
   ')|M*~Cobrada~C(0)@d6b@'),FROM(Queue:Browse:1)
                       BUTTON('Alta'),AT(301,175,53,32),USE(?Insert:2),SKIP,LEFT,ICON('C:\Comisiones\ComisionesSRL\botones\agregar.ico')
                       BUTTON('Modificar'),AT(358,175,53,15),USE(?Change:2),SKIP,LEFT,ICON('C:\Comisiones\ComisionesSRL\botones\modificar.ico')
                       BUTTON('Borrar'),AT(358,192,53,15),USE(?Delete:2),SKIP,LEFT,ICON('C:\Comisiones\ComisionesSRL\botones\borrar.ico')
                       BUTTON('Imprimir'),AT(104,192,52,15),USE(?Imprimir),SKIP,LEFT,KEY(F2Key),ICON('C:\Comisiones\ComisionesSRL\botones\imprimir.ICO')
                       BUTTON('Guías...'),AT(104,175,52,15),USE(?Aplicaciones),SKIP,LEFT,ICON('C:\Comisiones\ComisionesSRL\botones\ventana.ico')
                       SHEET,AT(7,30,409,182),USE(?CurrentTab),ABOVE(54)
                         TAB('Por &Fecha'),USE(?Tab:1)
                           PROMPT('Fecha:'),AT(18,177),USE(?FAC:Fecha:Prompt),TRN
                           ENTRY(@d6),AT(48,177,49,10),USE(FAC:Fecha),RIGHT(1),FONT(,,0A00000H,,CHARSET:ANSI)
                         END
                         TAB('Por &Número'),USE(?Tab:2)
                           PROMPT('Número:'),AT(18,177),USE(?FAC:Numero:Prompt),TRN
                           ENTRY(@n_8b),AT(48,177,49,10),USE(FAC:Numero),RIGHT(1)
                         END
                         TAB('Por &Cliente'),USE(?Tab:3)
                           BUTTON('Cliente...'),AT(159,175,52,15),USE(?SelecCLIENTE),SKIP,LEFT,ICON('C:\Comisiones\ComisionesSRL\botones\buscar.ico')
                         END
                       END
                       PANEL,AT(13,192,84,15),USE(?Panel2),FILL(0EDECE2H),BEVEL(-1)
                       STRING('Saldo:'),AT(18,195),USE(?String6),TRN
                       ...
                       BUTTON('Cobrada'),AT(223,192,52,15),USE(?Cobrada),SKIP,LEFT,ICON('C:\Comisiones\ComisionesSRL\botones\cobrado2.ico')
                       BUTTON('   Cerrar'),AT(185,217,53,15),USE(?Close),SKIP,RIGHT,ICON('C:\Comisiones\ComisionesSRL\botones\sale.ico')
                     END
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('FACTURAS_EMPRESA1')
  ...
  BRW1.SetFilter('(FAC:Empresa = 1 AND (FAC:Comprobante = ''FAC'' OR FAC:Comprobante = ''NC''))') ! Apply filter expression to browse
...
Looped BYTE
  ...
  LOOP                                                     ! This method receive all EVENT:Accepted's
    ...
    OF ?Change:2
      ...
      IF FAC:Cobrada THEN
        ...
        MESSAGE(' No es posible modificar la Factura,| la misma se encuentra Cobrada.','Atención !!!',ICON:Exclamation)
        CYCLE
      END
      IF FAC:Impresa = 'S' THEN
        BEEP(BEEP:SystemExclamation)  ;  YIELD()
        MESSAGE(' No es posible modificar la Factura,| la misma se encuentra Impresa.','Atención !!!',ICON:Exclamation)
        CYCLE
      END
      
      IF FAC:FE=1 THEN
        BEEP(BEEP:SystemExclamation)  ;  YIELD()
        MESSAGE(' No es posible borrar la Factura,| ya que la misma es electrónica.','Atención !!!',ICON:Exclamation)
        CYCLE
      END
    OF ?Delete:2
      BRW1.UpdateViewRecord
      IF FAC:Cobrada THEN
        BEEP(BEEP:SystemExclamation)  ;  YIELD()
        MESSAGE(' No es posible borrar la Factura,| la misma se encuentra Cobrada.','Atención !!!',ICON:Exclamation)
        CYCLE
      END
  ...
  ReturnValue = PARENT.TakeAccepted()
    ...
    OF ?Imprimir:2
      ThisWindow.Update
      ImpresionFacturaElectronica1
      ThisWindow.Reset
      BRW1.UpdateViewRecord
      !glo:FEImprimir=FAC:RegFactura
      !ImprimeFacturaElectronica
    OF ?Imprimir
      ThisWindow.Update
      ImpresionFactura1
      ThisWindow.Reset
    ...
    OF ?SelecCLIENTE
      ...
      IF CHOICE(?CurrentTab) = 3 THEN
        IF NOT(RECORDS(BRW1)) THEN
          ...
          MESSAGE('El Cliente seleccionado no registra movimientos.','Sin Movimientos',ICON:Question)
        END
      END
    OF ?Cobrada
      ThisWindow.Update
...
BRW1.SetQueueRecord PROCEDURE
  ...
  END
  SELF.Q.FAC:Comprobante_NormalFG = -1                     ! Set color values for FAC:Comprobante
  SELF.Q.FAC:Comprobante_NormalBG = -1
  SELF.Q.FAC:Comprobante_SelectedFG = -1
  SELF.Q.FAC:Comprobante_SelectedBG = -1
